 ...
 */
function fc_colony_apply_report(array $user, array $data): array
{
    ...
    fc_exec(
        'INSERT INTO fc_colony_haulers (market_id, user_id, cmdr, carrier, ship, cargo_capacity, updated_at)
    ...
    fc_exec(
        'DELETE FROM fc_colony_stock WHERE market_id = :m AND user_id = :u',
        ['m' => $marketId, 'u' => $user['id']],
    );
    if ($stock !== []) {
        $stmt = fc_db()->prepare(
            'INSERT INTO fc_colony_stock (market_id, user_id, commodity, in_ship, on_carrier, delivered)
        ...
        foreach ($stock as $row) {
            ...
            $stmt->execute([
                'm' => $marketId, 'u' => $user['id'], 'c' => mb_substr($commodity, 0, 64),
...
function fc_colony_view(array $site): array
{
    ...
    $haulers = fc_all(
        'SELECT h.*, u.username
           ...
           LEFT JOIN fc_users u ON u.id = h.user_id
          WHERE h.market_id = :id
    ...
    foreach ($haulers as $hauler) {
        $names[(int) $hauler['user_id']] = $hauler['cmdr'] ?: ($hauler['username'] ?? 'someone');
    }
    ...
    foreach ($needs as $need) {
        ...
        foreach ($rows as $row) {
            ...
            $who[] = [
                'cmdr' => $names[(int) $row['user_id']] ?? 'someone',
                'ship' => (int) $row['in_ship'],
                'carrier' => (int) $row['on_carrier'],
                'delivered' => (int) $row['delivered'],
            ];
